...
const guaranteeBadges = [{
  id: 'gb-1',
  label: 'Aantoonbaar ingericht'
}, {
  id: 'gb-2',
  label: 'Kosteloos ondersteund'
}, {
  id: 'gb-3',
  label: 'Klaar bij controle'
}] as any[];

// @component: GuaranteeSection
export const GuaranteeSection = () => {
  return <section id="garantie" className="pb-16 sm:pb-24 lg:pb-32 px-5 sm:px-8 bg-[#1E2238]">
      <div className="max-w-6xl mx-auto">
        <div className="bg-white/5 border border-white/10 rounded-[32px] sm:rounded-[40px] p-8 sm:p-12 md:p-16 flex flex-col md:flex-row items-center gap-8 sm:gap-10 overflow-hidden relative">
          {/* Background glow */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#E07B39]/8 blur-[100px] -mr-32 -mt-32 rounded-full" />

          {/* Badge seal */}
          <div className="flex-shrink-0 flex items-center justify-center">
            <div className="w-32 h-32 sm:w-40 sm:h-40 md:w-44 md:h-44 border-2 border-[#E07B39] rounded-full flex items-center justify-center relative">
              <div className="absolute inset-2 border border-[#E07B39]/30 rounded-full" />
              <div className="text-center flex flex-col items-center justify-center gap-1.5 relative z-10 px-3 sm:px-4">
                <ShieldCheck className="w-7 h-7 sm:w-8 sm:h-8 text-[#E07B39]" />
                <span className="font-black uppercase tracking-widest text-white leading-tight text-center" style={{
                fontSize: '10px'
              }}>
                  <span className="block text-xs">DBA</span>
          ...
          {/* Content */}
          <div className="flex-grow space-y-4 sm:space-y-5 text-center md:text-left">
            {/* Label */}
            <div className="flex items-center justify-center md:justify-start gap-3">
              <div className="w-10 h-[1px] bg-[#E07B39]" />
              <span className="text-[#E07B39] font-bold tracking-[0.3em] uppercase text-xs">Zekerheid</span>
            </div>
            ...
            <h2 className="text-[28px] sm:text-[40px] md:text-[48px] lg:text-[56px] font-serif text-white tracking-tight leading-[1.05]">
              ...
              fontFamily: 'Georgia, serif'
            }}>Ondersteuningsgarantie</span>
              <span className="text-[#E07B39]">.</span>
            </h2>

            {/* Body */}
            <p className="text-sm sm:text-base md:text-lg text-white/55 font-light leading-relaxed max-w-2xl">
              Wij helpen je jouw ondernemerschap zo in te richten dat je duidelijk kunt laten zien hoe je als zelfstandig ondernemer werkt. Mocht er later een vraag of controle ontstaan rondom jouw zzp-situatie onder de Wet DBA, dan helpen wij je{' '}
              <span className="text-white font-medium">kosteloos</span> met documentatie en uitleg zodat je jouw werkwijze kunt toelichten.
            </p>

            {/* Badges */}
            <div className="flex flex-wrap justify-center md:justify-start gap-2 sm:gap-3 pt-2">
              {guaranteeBadges.map(badge => <div key={badge.id} className="flex items-center gap-2 sm:gap-2.5 px-3 sm:px-4 py-2 sm:py-2.5 bg-[#E07B39]/10 border border-[#E07B39]/20 rounded-full">
                  <Check className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#E07B39] shrink-0" />
                  <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-white/80">{badge.label}</span>
                </div>)}
            </div>
          </div>
