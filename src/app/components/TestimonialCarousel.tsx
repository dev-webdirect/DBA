...
const testimonials: Testimonial[] = [{
  ...
  quote: 'Eindelijk een partij die snapt waar de angst van mijn opdrachtgevers vandaan komt. Binnen 5 dagen had ik een profiel dat direct alle twijfels wegnam bij inkoop.'
}, {
  id: 2,
  name: 'Sanne Meijer',
  role: 'Interim HR Specialist',
  quote: "Ik zag opdrachten aan mijn neus voorbijgaan door de wet DBA. Sinds ik 'DBA Klaar' ben, is het proces bij nieuwe klanten 10x makkelijker."
}, {
  id: 3,
  name: 'Mark de Boer',
  role: 'IT Consultant',
  quote: "De website en het certificaat geven me een enorme voorsprong. Het is geen 'moetje' meer, maar een marketinginstrument geworden."
}, {
  id: 4,
  name: 'Lisa Verhoeven',
  role: 'Freelance UX Designer',
  quote: 'Mijn opdrachtgever twijfelde intern over de inhuur. Nadat ik het DBA Klaar certificaat deelde, was die discussie meteen van tafel. Puur goud.'
}, {
  id: 5,
  name: 'Jeroen Smits',
  role: 'Zelfstandig Financieel Adviseur',
  quote: 'Ik dacht dat ik alles al op orde had. DBA Klaar liet zien wat er ontbrak. Nu sta ik professioneler dan ooit en heb ik al twee nieuwe opdrachten binnengehaald.'
}, {
  id: 6,
  name: 'Nadia el Amrani',
  role: 'Interim Communicatiespecialist',
  quote: 'Het hele traject duurde letterlijk vijf dagen. Ik hoefde niks te doen en had daarna een complete professionele uitstraling. Ongelooflijk efficiënt.'
}, {
  id: 7,
  name: 'Bas Kroon',
  role: 'Zelfstandig Bouwkundig Adviseur',
  quote: 'Opdrachtgevers in mijn sector zijn voorzichtiger geworden. Dankzij DBA Klaar kan ik nu aantonen dat mijn zelfstandigheid volledig op orde is. Dat geeft rust.'
}, {
  id: 8,
  name: 'Femke Dijkstra',
  role: 'Zelfstandig Marketingstrateeg',
  quote: 'De combinatie van website, LinkedIn en de samenwerkingsdocumenten werkt als een professioneel pakket. Opdrachtgevers zien direct dat ik serieus ondernemer ben.'
}];
const stats = [{
  value: '5',
  label: 'Dagen tot livegang'
}, {
  value: '7',
  label: 'deliverables in één pakket'
}, {
  value: '1',
  label: 'intake — wij doen de rest'
}] as any[];
const TestimonialCard = ({
  testimonial,
  isCenter
  ...
  testimonial: Testimonial;
  isCenter: boolean;
}) => <div className={cn('h-full w-full bg-[#1E2238] border rounded-[32px] sm:rounded-[40px] p-7 sm:p-10 md:p-12 flex flex-col justify-between relative overflow-hidden group transition-all duration-700', isCenter ? 'border-[#E07B39]/30 shadow-[0_20px_50px_rgba(224,123,57,0.08)] opacity-100 z-30' : 'border-white/5 shadow-none opacity-40 grayscale-[0.5] blur-[4px] z-10')}>
    <Quote className="absolute top-6 right-6 sm:top-8 sm:right-8 w-14 h-14 sm:w-20 sm:h-20 md:w-24 md:h-24 text-[#E07B39]/5 -z-0" />

    <div className="relative z-10 space-y-5 sm:space-y-8">
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map(s => <Star key={s} size={13} className="fill-[#E07B39] text-[#E07B39]" />)}
      </div>
      <p className={cn('font-serif text-white italic font-light leading-relaxed transition-all duration-700', isCenter ? 'text-lg sm:text-xl md:text-3xl' : 'text-base sm:text-lg md:text-xl')}>
        "{testimonial.quote}"
      </p>
    ...
    <div className="relative z-10 flex items-center mt-7 sm:mt-12">
      <div>
        <h4 className="text-base sm:text-lg md:text-xl font-bold text-white">{testimonial.name}</h4>
        <span className="text-[#E07B39] text-[10px] md:text-xs uppercase tracking-widest font-medium">
          {testimonial.role}
        </span>
      </div>
    </div>
...
export const TestimonialCarousel = () => {
  ...
  return <div className="w-full bg-[#1E2238] overflow-hidden">
      <div className="relative z-20 mx-auto px-5 sm:px-8 py-16 sm:py-20 md:py-28 max-w-[1380px]">
        ...
        <div className="grid grid-cols-3 gap-4 sm:gap-8 md:gap-12 max-w-4xl mx-auto mb-16 sm:mb-24 md:mb-32 border-t border-white/5 pt-10 sm:pt-12">
          {stats.map((stat, idx) => <div key={`stat-${idx}`} className="flex flex-col items-center text-center space-y-1 sm:space-y-2">
              <h3 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white tabular-nums">{stat.value}</h3>
              <p className="text-[#E07B39] text-[9px] sm:text-[10px] uppercase font-black tracking-widest leading-tight">{stat.label}</p>
            </div>)}
        </div>
        ...
        <div className="text-center mb-12 sm:mb-16 md:mb-20 space-y-5 sm:space-y-6">
          <div className="flex items-center justify-center gap-3">
            ...
            <span className="text-[#E07B39] font-bold tracking-[0.2em] uppercase text-xs">Testimonials</span>
            <div className="w-10 h-[1px] bg-[#E07B39]" />
          </div>
          <h2 className="text-[34px] sm:text-[48px] lg:text-[56px] xl:text-[72px] font-serif text-white tracking-tighter leading-none">
            <span>Onze klanten aan </span>
            <span className="italic font-light text-[#E07B39]" style={{
            fontFamily: 'Georgia, serif'
          }}>
              woord<span className="text-[#E07B39]">.</span>
            </span>
          </h2>
        </div>
        ...
        <div className="relative w-full max-w-7xl mx-auto overflow-visible">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6 sm:gap-8 mt-10 sm:mt-14 md:mt-16 px-2 sm:px-4">
            <div className="flex gap-3 sm:gap-4">
              <button onClick={handlePrev} className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full border border-white/10 flex items-center justify-center text-white hover:bg-[#E07B39] hover:text-white hover:border-[#E07B39] transition-all group active:scale-95" aria-label="Previous slide">
                <ChevronLeft className="w-5 h-5 sm:w-6 sm:h-6 group-hover:-translate-x-1 transition-transform" />
              </button>
              <button onClick={handleNext} className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full border border-white/10 flex items-center justify-center text-white hover:bg-[#E07B39] hover:text-white hover:border-[#E07B39] transition-all group active:scale-95" aria-label="Next slide">
                <ChevronRight className="w-5 h-5 sm:w-6 sm:h-6 group-hover:translate-x-1 transition-transform" />
              </button>
            </div>

            <div className="flex gap-2 sm:gap-3">
              {testimonials.map((_, i) => <button key={`dot-${i}`} onClick={() => goToSlide(i)} className={cn('h-1.5 transition-all duration-500 rounded-full', activeIndex === i ? 'w-8 sm:w-10 md:w-12 bg-[#E07B39]' : 'w-2 sm:w-3 bg-white/10 hover:bg-white/20')} aria-label={`Go to slide ${i + 1}`} />)}
            </div>
          </div>
        </div>
      </div>
